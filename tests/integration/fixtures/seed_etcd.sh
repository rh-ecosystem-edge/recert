...
DUMMY_CA_BUNDLE=$(printf '%s' "dummy-ca-data" | openssl base64 -e -A)
...
put "/kubernetes.io/config.openshift.io/clusterversions/version" '{
  ...
  "status": {
    "conditions": [
      {"type": "Available", "status": "True", "reason": "AsExpected", "message": "Done"}
    ]
  }
}'
...
put "/kubernetes.io/deployments/openshift-apiserver-operator/openshift-apiserver-operator" '{
  ...
  "spec": {
    "selector": {"matchLabels": {"app": "openshift-apiserver-operator"}},
    "template": {
      "metadata": {"labels": {"app": "openshift-apiserver-operator"}},
      "spec": {
        "containers": [{
          "name": "openshift-apiserver-operator",
          "image": "dummy-operator:latest",
...
for name in etcd-serving-ca trusted-ca-bundle audit-1; do
...
# Deployments must be valid apps/v1 objects: recert protobuf-encodes edited Deployments
# on commit_to_actual_etcd, so incomplete v1 stubs fail after postprocess.
...
put "/kubernetes.io/deployments/openshift-apiserver/apiserver" "{
  ...
  \"spec\": {
    \"selector\": {\"matchLabels\": {\"app\": \"openshift-apiserver\"}},
    \"template\": {
      \"metadata\": {
        \"labels\": {\"app\": \"openshift-apiserver\"},
        \"annotations\": {${APISERVER_ANNOTATIONS}}
      },
      \"spec\": {
        \"containers\": [{\"name\": \"openshift-apiserver\", \"image\": \"dummy-apiserver:latest\"}]
      }
    }
  }
}"
...
put "/kubernetes.io/deployments/openshift-oauth-apiserver/apiserver" "{
  ...
  \"spec\": {
    \"selector\": {\"matchLabels\": {\"app\": \"openshift-oauth-apiserver\"}},
    \"template\": {
      \"metadata\": {
        \"labels\": {\"app\": \"openshift-oauth-apiserver\"},
        \"annotations\": {${OAUTH_ANNOTATIONS}}
      },
      \"spec\": {
        \"containers\": [{
...
put "/kubernetes.io/secrets/openshift-kube-apiserver/webhook-authenticator-1" '{
  "apiVersion": "v1",
  "kind": "Secret",
  "metadata": {"name": "webhook-authenticator-1", "namespace": "openshift-kube-apiserver"},
  "data": {}
}'
put "/kubernetes.io/secrets/openshift-kube-apiserver/webhook-authenticator" '{
  "apiVersion": "v1",
  "kind": "Secret",
  "metadata": {"name": "webhook-authenticator", "namespace": "openshift-kube-apiserver"},
  "data": {}
}'
put "/kubernetes.io/secrets/openshift-config/webhook-authentication-integrated-oauth" '{
  "apiVersion": "v1",
  "kind": "Secret",
  "metadata": {"name": "webhook-authentication-integrated-oauth", "namespace": "openshift-config"},
  "data": {}
}'
