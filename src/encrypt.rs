...
const AES_KEY_SIZE_BYTES: usize = AES_KEY_SIZE / 8;
...
async fn generate_key() -> Result<Vec<u8>> {
    let output = Command::new("openssl")
        .arg("rand")
        .arg(AES_KEY_SIZE_BYTES.to_string())
        .output()
        .await
        ...
        output.stdout.len() == AES_KEY_SIZE_BYTES,
        format!("expected {} bytes, got {}", AES_KEY_SIZE_BYTES, output.stdout.len())
    );
