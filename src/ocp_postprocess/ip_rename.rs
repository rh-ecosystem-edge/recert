...
pub(crate) async fn rename_all_dual_stack(etcd_client: &Arc<InMemoryK8sEtcd>, ips: &[String], dirs: &[ConfigPath], files: &[ConfigPath]) -> Result<()> {
    let original_ips = extract_original_dual_stack_ips(etcd_client).await.context("extracting original dual-stack IPs")?;

    fix_etcd_resources_dual_stack(etcd_client, &original_ips, ips).await.context("modifying etcd resources for dual stack")?;
...
async fn fix_filesystem_resources_dual_stack(original_ips: &[String], ips: &[String], dirs: &[ConfigPath], files: &[ConfigPath]) -> Result<()> {
    for dir in dirs {
...
async fn fix_etcd_resources_for_ip_pair(
...
async fn fix_etcd_resources(etcd_client: &Arc<InMemoryK8sEtcd>, ip: &str) -> Result<String> {
    ...
    if original_ips.len() != 1 {
        bail!("Expected single-stack (1 IP) but found {} IPs", original_ips.len());
    }
...
async fn extract_original_dual_stack_ips(etcd_client: &Arc<InMemoryK8sEtcd>) -> Result<Vec<String>> {
    ...
    if original_ips.len() != 2 {
        bail!("Expected dual-stack (2 IPs) but found {} IPs", original_ips.len());
    }
...
async fn fix_etcd_resources_dual_stack(etcd_client: &Arc<InMemoryK8sEtcd>, original_ips: &[String], new_ips: &[String]) -> Result<()> {
    ...
    log::info!("Applying dual-stack IP changes - IPv4: {} → {}, IPv6: {} → {}", original_ipv4, new_ipv4, original_ipv6, new_ipv6);
