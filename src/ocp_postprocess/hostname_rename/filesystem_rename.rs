...
pub(crate) async fn fix_filesystem_kapi_startup_monitor_pod(hostname: &str, dir: &Path) -> Result<()> {
    join_all(file_utils::globvec(dir, "**/kube-apiserver-startup-monitor-pod.yaml")?.into_iter().map(|file_path| {
        ...
        tokio::spawn(async move {
            async move {
                ...
                rename_utils::fix_kapi_startup_monitor_pod_container_args(&mut pod, &hostname).context("fixing kube-apiserver-startup-monitor-pod.yaml")?;

                commit_file(file_path, serde_json::to_string(&pod).context("serializing kube-apiserver-startup-monitor-pod.yaml")?)
                    .await
            ...
            .await
            .context(format!("fixing kube-apiserver-startup-monitor-pod.yaml {:?}", kapi_startup_monitor_pod_path))
        })
    }))
    .await
    .into_iter()
...
pub(crate) async fn fix_filesystem_kapi_startup_monitor_configmap_pod_yaml(original_hostname: &str, hostname: &str, dir: &Path) -> Result<()> {
    join_all(file_utils::globvec(dir, "**/kube-apiserver-pod*/configmaps/kube-apiserver-pod/kube-apiserver-startup-monitor-pod.yaml")?.into_iter().map(|file_path| {
        let kapi_startup_monitor_pod_path = file_path.clone();
        let original_hostname = original_hostname.to_string();
        let hostname = hostname.to_string();
        tokio::spawn(async move {
            async move {
                ...
                commit_file(
                    file_path,
                    rename_utils::fix_kapi_startup_monitor_pod_yaml(&contents, &original_hostname, &hostname).context("fixing kube-apiserver-startup-monitor-pod.yaml")?,
                )
                .await
                .context("writing kube-apiserver-startup-monitor-pod.yaml to disk")?;
