...
// Helper function to determine if an IP is IPv4 or IPv6
...
async fn extract_original_ips_from_nodes(etcd_client: &Arc<InMemoryK8sEtcd>) -> Result<Vec<String>> {  
    ...
    ensure!(!node_keys.is_empty(), "No nodes found in etcd");
    
    // Get the first (and should be only) node
    let node_key = &node_keys[0];

    let etcd_result = etcd_client.get(node_key.clone()).await?
        .context("Failed to get node from etcd")?;

    let node: Value = serde_yaml::from_slice(&etcd_result.value)
        .context("Failed to deserialize node value")?;
    
    let addresses = node.pointer("/status/addresses")
...
pub(crate) async fn fix_openshift_apiserver_configmap(etcd_client: &Arc<InMemoryK8sEtcd>, original_ip: &str, ip: &str) -> Result<()> {
    ...
    if current_url == expected_original_url {
        ...
        log::info!("Original IP {} not found in openshift apiserver configmap, current URL is {}, skipping replacement", original_ip, current_url);
    }
...
fn fix_storage_config(config: &mut Value, original_ip: &str, ip: &str) -> Result<()> {
    ...
    let current_urls = storage_config.pointer("/urls")
        .and_then(|urls| urls.as_array())
        .context("storageConfig/urls not found or not an array")?;
    
    let original_ip_formatted = if original_ip.contains(':') { format!("[{original_ip}]") } else { original_ip.to_string() };
    let expected_url = format!("https://{original_ip_formatted}:2379");
    
    // Only replace if the original IP is found in the URLs
    let contains_original = current_urls.iter().any(|url| {
        url.as_str().map_or(false, |s| s == expected_url)
    });
        ...
        etcd_client
            ...
            .map(|key| async move {
                ...
                if current_value == original_ip {
                    ...
                    log::info!("Original IP {} not found in etcd endpoints, current value is {}, skipping replacement", original_ip, current_value);
                }
    ...
    containers
        ...
        .try_for_each(|container| {
            ...
            let Some(arg_idx) = arg_idx else {
                log::info!("etcd server argument with {} not found, skipping (this is normal for IPv6 in dual-stack)", original_ip);
                return Ok(());
            };
    ...
    if let Some(annotations) = cluster.pointer_mut("/metadata/annotations") {
        ...
        if let Some(current_value) = annotations.get(key) {
            ...
            if current_value_str == original_ip {
                ...
                log::info!("Original IP {} not found in networks cluster annotation, current value is {}, skipping replacement", original_ip, current_value_str);
            }
        } else {
...
pub(crate) async fn fix_etcd_member(etcd_client: &Arc<InMemoryK8sEtcd>, original_ip: &str, ip: &str) -> Result<()> {
    ...
    if contains_original {
        ...
        etcd_client.update_member(new_member_url).await.context("failed to update etcd member")?;
    } else {
        log::info!("Original IP {} not found in etcd member peer URLs, current URLs are {:?}, skipping replacement", 
                   original_ip, current_member.peer_urls());
    }
