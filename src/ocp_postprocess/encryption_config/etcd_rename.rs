    ...
    join_all(
        etcd_client
            ...
            .map(|key| async move {
                ...
                let encryption_key_base64 = base64_standard.encode(encryption_key.clone());

                if *data.get(ENCRYPTION_KEY_NAME).context("cound not find encryption-key")?.to_string() != encryption_key_base64 {
                    data.insert(
