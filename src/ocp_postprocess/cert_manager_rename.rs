...
pub(crate) async fn fix_cert_manager_certificates(
    ...
    join_all(cert_keys.into_iter().map(|key| async move {
        ...
        if apply_cn_san_replace_to_certificate(&mut value, cn_san_replace_rules) {
            etcd_client
                .put(&key, serde_json::to_vec(&value)?, None)
                .await
...
fn apply_cn_san_replace_to_certificate(value: &mut serde_json::Value, cn_san_replace_rules: &CnSanReplaceRules) -> bool {
    ...
    for &field in ARRAY_FIELDS {
        modified |= replace_string_array(value, field, cn_san_replace_rules);
    }

    modified
}

fn replace_string_array(value: &mut serde_json::Value, (pointer, label): (&str, &str), cn_san_replace_rules: &CnSanReplaceRules) -> bool {
    let mut modified = false;
...
mod tests {
    ...
    #[test]
    fn test_replaces_all_five_fields_simultaneously() {
        let rules = make_rules(&[
            ("seed.example.com", "target.example.com"),
            ("10.0.0.1", "10.0.0.2"),
            ("spiffe://seed.cluster.local", "spiffe://target.cluster.local"),
            ("admin@seed.example.com", "admin@target.example.com"),
        ]);
        let mut cert = make_certificate_full(
            "seed.example.com",
            &["seed.example.com"],
            &["10.0.0.1"],
            &["spiffe://seed.cluster.local"],
        ...
        assert_eq!(
            cert.pointer("/spec/emailAddresses/0").unwrap().as_str().unwrap(),
            "admin@target.example.com"
        );
    }

    #[test]
    fn test_common_name_non_string_skipped() {
        let rules = make_rules(&[("seed.example.com", "target.example.com")]);
        let mut cert = json!({
            "apiVersion": "cert-manager.io/v1",
            "kind": "Certificate",
        ...
        assert!(modified);
        assert_eq!(cert.pointer("/spec/commonName").unwrap().as_i64().unwrap(), 42);
        assert_eq!(cert.pointer("/spec/dnsNames/0").unwrap().as_str().unwrap(), "target.example.com");
    }

    #[test]
    fn test_non_string_array_element_skipped() {
        let rules = make_rules(&[("seed.example.com", "target.example.com")]);
        let mut cert = json!({
            "apiVersion": "cert-manager.io/v1",
            "kind": "Certificate",
        ...
        assert!(modified);
        assert_eq!(cert.pointer("/spec/dnsNames/0").unwrap().as_i64().unwrap(), 42);
        assert_eq!(cert.pointer("/spec/dnsNames/1").unwrap().as_str().unwrap(), "target.example.com");
    }

    #[test]
    fn test_missing_optional_fields() {
        let rules = make_rules(&[("seed.example.com", "target.example.com")]);
        let mut cert = make_certificate_full("seed.example.com", &[], &[], &[], &[]);
