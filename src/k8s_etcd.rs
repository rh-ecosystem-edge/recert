...
impl InMemoryK8sEtcd {
    ...
    pub(crate) async fn put(&self, key: &str, value: Vec<u8>, encoding: Option<Encoding>) -> Result<()> {
        {
            ...
            let encoding = match encoding {
                ...
                None => {
                    let (enc, _) = hashmap
                        .get(key)
                        .context(format!("cannot determine encoding for key '{}': not yet in cache", key))?;
                    *enc
                }
            };
    ...
    pub(crate) async fn encoding_of(&self, key: &str) -> Result<Encoding> {
        let hashmap = self.etcd_keyvalue_hashmap.lock().await;
        let (encoding, _) = hashmap
            .get(key)
            .context(format!("cannot determine encoding for key '{}': not yet in cache", key))?;
        Ok(*encoding)
    }
...
pub(crate) async fn put_etcd_yaml(client: &InMemoryK8sEtcd, k8slocation: &K8sResourceLocation, value: Value) -> Result<()> {
    client
        .put(&k8slocation.as_etcd_key(), serde_json::to_string(&value)?.as_bytes().into(), None)
        .await
...
mod tests {
    ...
    fn test_resource_from_key_short_key_is_none() {
        assert_eq!(resource_from_key(""), None);
        assert_eq!(resource_from_key("secrets"), None);
        assert_eq!(resource_from_key("/"), None);
